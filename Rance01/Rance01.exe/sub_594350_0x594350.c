...
if (hHandle != 0 && eax == WAIT_TIMEOUT)
    HANDLE hEvent = *(arg1 + 0x44)
    
    if (hEvent != 0)
        SetEvent(hEvent)
    
    enum WAIT_EVENT eax_4 = WaitForSingleObject(*(arg1 + 0x40), 0x1388)
    
    if (eax_4 == WAIT_TIMEOUT)
        sub_40da60("CDummySoundBuffer::StopFade()\n")
    else if (eax_4 == WAIT_ABANDONED)
        sub_40da60("CDummySoundBuffer::StopFade()\n")
    
    HANDLE hEvent_1 = *(arg1 + 0x44)
