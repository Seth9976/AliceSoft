// 函数: sub_51a360
// 地址: 0x51a360
// 来自: E:\torrent\AliceSoft\ランス01\Rance01.exe

long double x87_r7 = float.t(1)
float var_84 = fconvert.s(x87_r7)
long double x87_r6 = float.t(0)
float var_80 = fconvert.s(x87_r6)
float var_7c = fconvert.s(x87_r6)
float var_78 = fconvert.s(x87_r6)
float var_74 = fconvert.s(x87_r6)
float var_6c = fconvert.s(x87_r6)
float var_68 = fconvert.s(x87_r6)
float var_64 = fconvert.s(x87_r6)
float var_60 = fconvert.s(x87_r6)
float var_58 = fconvert.s(x87_r6)
float var_70 = fconvert.s(x87_r7)
float var_5c = fconvert.s(x87_r7)
float var_54 = fconvert.s(fconvert.t(*(arg1 + 0x14)))
float var_50 = fconvert.s(fconvert.t(*(arg1 + 0x18)))
float var_44[0x11]
float (* eax)[0x11] = &var_44
float var_4c = fconvert.s(fconvert.t(*(arg1 + 0x1c)))
float var_48 = fconvert.s(x87_r7)
int32_t edx
sub_660a50(eax, edx, arg1 + 4)
sub_4b65f0(eax, &var_84, &var_44, arg2)
return arg2
