        ...
        if (eax_4 + eax s>= 0)
            ...
            if (eax_5 s< (*(arg1 + 0x18) - ecx_2) s>> 4)
                ...
                for (int32_t edi_1 = *(ecx_2 + (eax_6 << 3) + 4); i != edi_1; i = &i[1])
                    void* esi_1 = *i
                    int80_t st0_1
                    st0_1, eax_6 = sub_4f1950(esi_1, fconvert.s(fconvert.t(arg2)), 
                        fconvert.s(fconvert.t(arg3)))
                    
                    if (eax_6.b != 0)
                        long double x87_r7_11 = fconvert.t(fconvert.s((
                            fneg(fconvert.t(*(esi_1 + 0x3c))) * fconvert.t(arg2)
                            - fconvert.t(*(esi_1 + 0x44)) * fconvert.t(arg3)
                            - fconvert.t(*(esi_1 + 0x48))) / fconvert.t(*(esi_1 + 0x40))))
